...
// Primeiro, vamos testar a conexão básica
try {
    $testResponse = $client->request('GET', 'Lead', ['maxSize' => 1]);
...
foreach ($emails as $email) {
    try {
        ...
        $status = "⚠️ Erro na consulta: " . $errorMsg;
        echo $status . "\n";
        $results[$email] = [
            'status' => 'erro',
            'id' => null,
...
$erros = array_filter($results, function($r) { return $r['status'] === 'erro'; });
