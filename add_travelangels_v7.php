...
logWithTimestamp($logs, "=== INÍCIO PROCESSAMENTO V7 ===");
...
// ===== PROCESSAMENTO TRAVELANGELS =====
logWithTimestamp($logs, "--- PROCESSANDO TRAVELANGELS ---");

// Tentar criar lead no TravelAngels
try {
    ...
    if (
        ...
        logWithTimestamp($logs, "TravelAngels - Lead duplicado detectado - buscando por email: " . $email);

        $existingLead = findLeadByEmail($email, $client, $logs);
        if ($existingLead) {
            logWithTimestamp($logs, "TravelAngels - Lead encontrado - atualizando: " . $existingLead['id']);

            // Atualizar lead existente
            $updateResponse = $client->request('PATCH', 'Lead/' . $existingLead['id'], $payload);
            logWithTimestamp($logs, "TravelAngels - Lead atualizado com sucesso");
            $leadIdTravelAngels = $existingLead['id'];
        } else {
            ...
            if (strpos($errorMessage, '"id":') !== false && strpos($errorMessage, '"name":') !== false) {
                ...
                if (isset($leadData[0]['id'])) {
                    logWithTimestamp($logs, "TravelAngels - Usando lead existente da resposta: " . $leadData[0]['id']);
                    $leadIdTravelAngels = $leadData[0]['id'];
                } else {
                    logWithTimestamp($logs, "TravelAngels - Erro: Lead duplicado mas não encontrado por email");
                    throw $e;
                }
            } else {
                logWithTimestamp($logs, "TravelAngels - Erro: Lead duplicado mas não encontrado por email");
        ...
        logWithTimestamp($logs, "TravelAngels - Erro real na criação do lead: " . $errorMessage);
        throw $e;
    }
}

// Tentar criar oportunidade no TravelAngels
if ($leadIdTravelAngels) {
    try {
        $opportunityPayload = [
            'name' => $name,
            'leadId' => $leadIdTravelAngels,
        ...
        logWithTimestamp($logs, "TravelAngels - Oportunidade criada com sucesso: " . $responseOpportunity['id']);
    } catch (Exception $e) {
        $errorMessage = $e->getMessage();
        logWithTimestamp($logs, "TravelAngels - Exceção oportunidade: " . $errorMessage);
        ...
        if (strpos($errorMessage, '409') !== false || strpos($errorMessage, 'duplicate') !== false) {
            logWithTimestamp($logs, "TravelAngels - Oportunidade duplicada detectada - criando nova com duplicate = yes");

            $opportunityPayload['duplicate'] = 'yes';
            $responseOpportunity = $client->request('POST', 'Opportunity', $opportunityPayload);
            logWithTimestamp($logs, "TravelAngels - Nova oportunidade criada com duplicate = yes: " . $responseOpportunity['id']);
        } else {
            logWithTimestamp($logs, "TravelAngels - Erro real na criação da oportunidade: " . $errorMessage);
        }
    }
}
...
logWithTimestamp($logs, "=== FIM PROCESSAMENTO V7 ===");
...
echo json_encode([
    'status' => 'success',
    'message' => 'Lead e oportunidade processados com sucesso no TravelAngels e FlyingDonkeys',
