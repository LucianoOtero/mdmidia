...
// Função para corrigir JSON malformado do Webflow API V2
...
logWithTimestamp($logs, "=== INÍCIO PROCESSAMENTO TRAVELANGELS V10 (API V2 CORRIGIDA) ===");
logWithTimestamp($logs, "Recebido do Webflow: " . substr($json, 0, 500) . "...");

// Tentar decodificar JSON com correção automática
$data = json_decode($json, true);

if (json_last_error() !== JSON_ERROR_NONE) {
    logWithTimestamp($logs, "JSON malformado detectado - tentando correção automática");
    $data = fixWebflowJson($json);
    
    if ($data === null) {
        logWithTimestamp($logs, "ERRO: Não foi possível corrigir o JSON malformado");
        fclose($logs);
        http_response_code(400);
        echo json_encode(['error' => 'JSON malformado - não foi possível corrigir']);
        exit;
    }
    
    logWithTimestamp($logs, "JSON corrigido com sucesso");
}

logWithTimestamp($logs, "Transformado em PHP Object: " . json_encode($data, JSON_UNESCAPED_UNICODE));
...
logWithTimestamp($logs, "=== FIM PROCESSAMENTO TRAVELANGELS V10 (API V2 CORRIGIDA) ===");
...
echo json_encode([
    'status' => 'success',
    'message' => 'Lead processado no TravelAngels e FlyingDonkeys com sucesso (API V2 corrigida)',
