...
{
    echo "\n🔍 BUSCANDO OPORTUNIDADES POR EMAIL:\n";
    echo "   Email: $email\n";
    ...
    if ($lead && isset($lead['id'])) {
        ...
        echo "   ❌ Lead não encontrado - não é possível buscar oportunidades\n";
        return [];
    }
}
...
function gerarRelatorioEleganteCompleto($dados, $resultado_webhook, $logs_webhook, $registros_espocrm)
{
    echo "\n" . str_repeat("=", 80) . "\n";
...
// 5) Consultar registros no EspoCRM (LEAD + OPORTUNIDADE)
$registros_espocrm = consultarRegistrosEspoCrmCompleto($dados['email'], $dados['nome']);

// 6) Gerar relatório elegante e completo
gerarRelatorioEleganteCompleto($dados, $resultado_webhook, $logs_webhook, $registros_espocrm);
