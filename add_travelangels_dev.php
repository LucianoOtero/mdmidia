...
$raw_input = stripslashes($raw_input); // Corrigir escape duplo

// Função para corrigir JSON malformado de forma simples e eficaz
function fixMalformedJson($json_string) {
    // Remover espaços extras
    $fixed = trim($json_string);
    
    // Adicionar aspas nas chaves
    $fixed = preg_replace('/([{,]\s*)([a-zA-Z_][a-zA-Z0-9_]*)\s*:/', '$1"$2":', $fixed);
    
    // Adicionar aspas nos valores que não são números
    $fixed = preg_replace('/:\s*([^"0-9][^,}]*?)\s*([,}])/', ': "$1"$2', $fixed);
    
    return $fixed;
}

$raw_input = fixMalformedJson($raw_input);
...
try {
    // Simular cliente CRM para desenvolvimento
    $client = new class {
    ...
    // Verificar se CRM está disponível (simulação)
    $crm_available = true; // Em desenvolvimento, sempre disponível
