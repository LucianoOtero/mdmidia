...
echo "Testando JSON exato do Webflow:\n";
echo "================================\n";

// Simular o que acontece no webhook dev
$data = json_decode($webflow_json, true);

if (json_last_error() !== JSON_ERROR_NONE) {
    echo "❌ ERRO no JSON principal: " . json_last_error_msg() . "\n";
    echo "Código do erro: " . json_last_error() . "\n\n";
    
    // Tentar decodificar o payload
    if (isset($data['payload'])) {
        echo "Tentando decodificar o payload...\n";
        $payload_data = json_decode($data['payload'], true);
        
        if (json_last_error() !== JSON_ERROR_NONE) {
            echo "❌ ERRO no payload: " . json_last_error_msg() . "\n";
            echo "Payload bruto: " . substr($data['payload'], 0, 200) . "...\n";
        } else {
